...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dist = (torch.sum((x_train[i, :] - x_test[j, :])**2))
        dists[i, j] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      dists[i, :] = (
        torch.sum(
          (x_test - x_train[i, :])**2, dim=tuple(range(1, len(x_train.shape)))
        )
      )
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = torch.sum(x_train**2, dim=1).reshape(-1, 1) + \
              torch.sum(x_test **2, dim=1).reshape(1, -1)- \
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    label for each test bisample by taking a MAJORITY VOTE among its `k` nearest
    ...
    for i in range(num_test):
      topk = torch.topk(dists[:, i], k, largest=False)
      labels = y_train[topk.indices]
      votted_label, _ = torch.mode(labels)
      y_pred[i] = votted_label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for test_idx in range(num_folds):
        x_train_k = torch.concat(x_train_folds[0:test_idx] + x_train_folds[(test_idx + 1):], dim = 0)
        y_train_k = torch.concat(y_train_folds[0:test_idx] + y_train_folds[(test_idx + 1):], dim=0)
        classifier = KnnClassifier(x_train_k, y_train_k)
        accuracy = classifier.check_accuracy(x_train_folds[test_idx], y_train_folds[test_idx], k)
        accur_lst.append(accuracy)
      k_to_accuracies[k] = accur_lst
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
      avg = sum(k_to_accuracies[k]) / len(k_to_accuracies[k])
      if max_val < avg:
        max_val = avg
        best_k = k
    ##########################################################################
