...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for ele in stem_channels:
            print("the element in stem_channels is ", ele)
        stem_cls = [
            nn.Conv2d(in_channels, stem_channels[0], 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(stem_channels[0], stem_channels[1], 3, padding=1),
            nn.ReLU(),
        ]
        stem_box = [
            nn.Conv2d(in_channels, stem_channels[0], 3, padding=1),
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feats in feats_per_fpn_level.items():
            class_logits[level_name] = self.pred_cls(self.stem_cls(feats))
            class_logits[level_name] = class_logits[level_name].reshape(
                class_logits[level_name].shape[0], -1, class_logits[level_name].shape[1]
            )
            boxreg_deltas[level_name] = self.pred_box(self.stem_box(feats))
            boxreg_deltas[level_name] = boxreg_deltas[level_name].reshape(
                boxreg_deltas[level_name].shape[0], -1, boxreg_deltas[level_name].shape[1]
            )
            centerness_logits[level_name] = self.pred_ctr(self.stem_box(feats))
            centerness_logits[level_name] = centerness_logits[level_name].reshape(
                centerness_logits[level_name].shape[0], -1, centerness_logits[level_name].shape[1]
            )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    x2 = gt_boxes[:, 2]
    y2 = gt_boxes[:, 3]
    deltas = torch.stack([x - x1, y - y1, x2 - x, y2 - y], dim=1) / stride
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for matched_gt_box in matched_gt_boxes:
            # matched_gt_box contains a dictionary with keys {"p3", "p4", "p5"}
            append_dict = {
                level_name: fcos_get_deltas_from_locations(
                    locations_per_fpn_level[level_name],
                    matched_gt_box[level_name],
                    self.backbone.fpn_strides[level_name],
                )
                for level_name in matched_gt_box.keys()
            } 
            matched_gt_deltas.append(append_dict)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        one_hot = F.one_hot(matched_gt_boxes[:, :, 4].long() + 1, self.num_classes + 1)
        one_hot = one_hot[:, :, 1:]
        one_hot = one_hot.to(dtype = pred_cls_logits.dtype, 
        ...
        # compute the loss box
        loss_box = 0.25 * F.l1_loss(
            pred_boxreg_deltas, matched_gt_deltas, reduction="none"
        )
        loss_box[matched_gt_deltas < 0] *= 0
        # compute the loss centerrness
        gt_centerness = torch.zeros_like(pred_ctr_logits)
        for j in range(gt_centerness.shape[1]):
            gt_centerness[:, j, :] = fcos_make_centerness_targets(matched_gt_deltas[:, j, :]).unsqueeze(1)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            idx = torch.argmax(level_pred_scores, dim=1)
            level_pred_scores = level_pred_scores[torch.arange(level_pred_scores.shape[0]), idx]
            # Step 2:
            # Replace "pass" statement with your code
            mask = level_pred_scores > test_score_thresh
            level_pred_scores = level_pred_scores[mask]
            level_pred_classes = idx[mask]
            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(
                level_deltas[mask], level_locations[mask], self.backbone.fpn_strides[level_name]
            )

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            h, w = images.shape[2:]
            level_pred_boxes[:, 0] = level_pred_boxes[:, 0].clamp(min=0, max=w)
            level_pred_boxes[:, 1] = level_pred_boxes[:, 1].clamp(min=0, max=h)
            level_pred_boxes[:, 2] = level_pred_boxes[:, 2].clamp(min=0, max=w)
            level_pred_boxes[:, 3] = level_pred_boxes[:, 3].clamp(min=0, max=h)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
