...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params["lateral_c3"] = nn.Conv2d(
            in_channels=64, out_channels=self.out_channels, kernel_size=1, stride=1
        )
        self.fpn_params["lateral_c4"] = nn.Conv2d(
            in_channels=160, out_channels=self.out_channels, kernel_size=1, stride=1
        )
        self.fpn_params["lateral_c5"] = nn.Conv2d(
            in_channels=400, out_channels=self.out_channels, kernel_size=1, stride=1
        )
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats["p4"] = fpn_feats["p4"] + F.interpolate(fpn_feats["p5"], size = fpn_feats["p4"].shape[2:], mode="nearest")
        fpn_feats["p3"] = fpn_feats["p3"] + F.interpolate(fpn_feats["p4"], size = fpn_feats["p3"].shape[2:], mode="nearest")
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        location_coords[level_name] = torch.stack([x.flatten(), y.flatten()], dim=1) 
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while idx.numel() > 0:
        ...
        idx = idx[1:]
        box = boxes[i, :]
        rest_boxes = boxes[idx, :]
        # calculate the iou
        iou = torch.zeros(rest_boxes.shape[0])
        x1 = torch.max(box[0], rest_boxes[:, 0])
        y1 = torch.max(box[1], rest_boxes[:, 1])
        x2 = torch.min(box[2], rest_boxes[:, 2])
        y2 = torch.min(box[3], rest_boxes[:, 3])
