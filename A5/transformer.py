...
def scaled_dot_product_two_loop_single(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros(query.shape, dtype = query.dtype, device=query.device)
    K, M = query.shape
    for i in range(K):
        weights = torch.zeros(K, dtype = query.dtype, device=query.device)
        for j in range(K):
            weights[j] = torch.dot(query[i], key[j]) / math.sqrt(M)
        weights = F.softmax(weights, dim = 0) 
        weighted_sum = (weights.unsqueeze(1) * value).sum(dim = 0)
        out[i] = weighted_sum
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    out = torch.zeros(query.shape, dtype = query.dtype, device=query.device)
    for i in range(K):
        weights = torch.zeros(N, K, dtype = query.dtype, device=query.device)
        for j in range(K):
            result = torch.bmm(query[:, i].unsqueeze(1), 
                                      key[:, j].unsqueeze(2)).squeeze(1).squeeze(1) / math.sqrt(M)
            weights[:, j] = result
        weights = F.softmax(weights, dim = 1)
        weighted_sum = torch.bmm(weights.unsqueeze(1), value).squeeze(1)
        out[:, i, :] = weighted_sum
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.attention = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)])
        self.linear = nn.Linear(num_heads * dim_out, dim_in)
        self.linear.weight.data.uniform_(-math.sqrt(6/(num_heads * dim_out + dim_in)), math.sqrt(6/(num_heads * dim_out + dim_in)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for head in self.attention:
            attention_out.append(head.forward(query, key, value, mask))
        y = self.linear(torch.cat(attention_out, dim = 2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        std = torch.sqrt((x - mean).pow(2).mean(dim = -1, keepdim = True) + self.epsilon)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.mlp1.weight.data.uniform_(-math.sqrt(6/(inp_dim + hidden_dim_feedforward)), math.sqrt(6/(inp_dim + hidden_dim_feedforward)))
        self.mlp2.weight.data.uniform_(-math.sqrt(6/(hidden_dim_feedforward + inp_dim)), math.sqrt(6/(hidden_dim_feedforward + inp_dim)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.attention.forward(x, x, x)
        out2 = self.dropout(self.norm1(out1 + x))
        out3 = self.feedforward.forward(out2) 
...
def get_subsequent_mask(seq):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    mask = torch.ones(seq.shape[0], seq.shape[1], seq.shape[1], dtype = torch.bool, device=seq.device)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out3 = self.attention_cross.forward(out2, enc_inp, enc_inp)
        out4 = self.dropout(self.norm2(out3 + out2))
        out5 = self.feed_forward.forward(out4)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = y.repeat(M, 1).transpose(1, 0).unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for i in range(M):
        if i % 2 == 0:
            y[:, i] = torch.sin(torch.arange(0, K) / (10000 ** (i // M)))
        else:
            y[:, i] = torch.cos(torch.arange(0, K) / (10000 ** (i // M)))
    y = y.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        K = a_emb_inp.shape[1]
        encoder_output = self.encoder.forward(q_emb_inp)
        mask = get_subsequent_mask(ans_b)
        mask = mask[:, :K, :K]
        dec_out = self.decoder.forward(a_emb_inp, encoder_output, mask)
