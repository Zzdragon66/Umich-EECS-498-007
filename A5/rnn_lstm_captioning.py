...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        if t == 0:
            values[t], cache[t] = rnn_step_forward(x[:, t, :], h0, Wx, Wh, b)
        else:
            values[t], cache[t]= rnn_step_forward(x[:, t, :], values[t-1], Wx, Wh, b) 
    h = torch.stack(values, dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return h, cache
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        nn.init.kaiming_normal_(self.image_proj.weight, mode='fan_out', nonlinearity='relu')
        self.image_proj.bias.data.fill_(0)
        # word extraction
        self.word_embedding = WordEmbedding(vocab_size, wordvec_dim)
        self.W_proj = nn.Linear(hidden_dim, vocab_size)
        nn.init.kaiming_normal_(self.W_proj.weight, mode='fan_out', nonlinearity='relu')
        self.W_proj.bias.data.fill_(0)
        # RNN
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "rnn" or self.cell_type == "lstm":
            ...
            reshaped_image_feats = torch.mean(image_feats, dim=(2, 3))
            h0 = self.image_proj(reshaped_image_feats)
            # step 2
            x = self.word_embedding(captions_in)
            # step 3
            h = self.rnn.forward(x, h0)
            # step 4
            scores = self.W_proj(h)
            # step 5
            loss = temporal_softmax_loss(scores, captions_out, ignore_index=self.ignore_index)
        elif self.cell_type == "attn":
            image_feats = self.image_encoder(images)
            A = image_feats
            A_reshaped = A.permute(0, 2, 3, 1).reshape(A.shape[0], -1, A.shape[1])
            A_mapped_reshaped = self.image_proj(A_reshaped)
            A_mapped = A_mapped_reshaped.reshape(A.shape[0], A.shape[2], A.shape[3], self.hidden_dim).permute(0, 3, 1, 2)

            #A = image_feats
            # step 2
            x = self.word_embedding(captions_in)
            # step 3
            h = self.rnn.forward(x, A_mapped)
            # step 4
            scores = self.W_proj(h)
            # step 5
            loss = temporal_softmax_loss(scores, captions_out, ignore_index=self.ignore_index)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "rnn":
            ...
            prev_h = h0
            # step 3
            prev_word = torch.tensor([self._start] * N, dtype=torch.long, device=images.device)
            for t in range(max_length):
                x = self.word_embedding(prev_word)
                # step 4
                next_h = self.rnn.step_forward(x, prev_h)
                scores = self.W_proj(next_h)
                prev_h = next_h
            ...
            for t in range(max_length):
                x = self.word_embedding(prev_word)
                # step 4
                attn, attn_weights = dot_product_attention(prev_h, A_mapped)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        i, f, o, g = torch.sigmoid(a[:, :H]), torch.sigmoid(a[:, H:2*H]), torch.sigmoid(a[:, 2*H:3*H]), torch.tanh(a[:, 3*H:])
        next_c = f * prev_c + i * g
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
            if t == 0:
                hn[t], prev_c = self.step_forward(x[:, t, :], h0, c0)
            else:
                hn[t], prev_c = self.step_forward(x[:, t, :], hn[t-1], prev_c)
...
def dot_product_attention(prev_h, A):
    ...
    M_att = torch.bmm(prev_h, A) / math.sqrt(H) # (N, 1, 16)
    M_att_flatten = M_att.view(N, -1) # (N, 16)
    attn_weights = F.softmax(M_att_flatten, dim=1) # (N, 16) 

    attn = A.bmm(attn_weights.unsqueeze(2)) # (N, H, 16) * (N, 16, 1) = (N, H, 1)
    attn = attn.squeeze(2) # (N, H)
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            if t == 0:
                ...
                hn[t], prev_c = self.step_forward(x[:, t, :], h0, c0, attn)
            else:
                attn, _ = dot_product_attention(hn[t-1], A)
                hn[t], prev_c = self.step_forward(x[:, t, :], hn[t-1], prev_c, attn)
        hn = torch.stack(hn, dim=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
