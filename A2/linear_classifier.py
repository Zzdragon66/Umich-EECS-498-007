...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores = X.matmul(W).cuda()
    y_pred = torch.argmax(scores, dim=1).cuda()
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = [1, 2, 3, 4, 5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_result = (cls.predict(data_dict['X_train']) == data_dict['y_train']).double()
    value_result = (cls.predict(data_dict['X_val']) == data_dict['y_val']).double()
    train_acc = (sum(train_result) / len(train_result)).item()
    val_acc = (sum(value_result) / len(value_result)).item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        dominator = torch.sum(torch.exp(scores))
        numerator = torch.exp(scores[y[i]])
        loss += -torch.log(numerator / dominator) 
        for j in range(num_classes):
            dW[:, j] += X[i] * torch.exp(scores[j]) / dominator
        dW[:, y[i]] -= X[i]
    loss /= num_train
    dW /= num_train
    dW += 2 * reg * W
...
def softmax_loss_vectorized(
    ...
    dominator = torch.sum(torch.exp(scores), dim=1)
    numerator = torch.exp(scores[range(num_train), y])
    loss = -torch.log(numerator / dominator).sum()
    loss /= num_train
    loss += reg * torch.sum(W * W)
    # the gradient
    ...
    dW -= X.T.matmul(mask)
    dW += X.T.matmul(torch.exp(scores) / dominator.view(-1, 1))
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
    ...
    regularization_strengths = [1e0, 1e1, 1e2, 1e3, 1e4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
