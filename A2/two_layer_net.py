...
def nn_forward_backward(
    ...
    correct_exp_scores = exp_scores[torch.arange(N), y]
    denominator = torch.sum(exp_scores, dim=1)
    probs = correct_exp_scores / denominator
    loss = torch.sum(-torch.log(probs)) / N + reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # Compute the w2 gradient
    dscores = exp_scores / denominator.view(-1,1)
    dscores[torch.arange(N), y] -= 1
    dscores /= N   
    grads['W2'] = h1.T.matmul(dscores) + 2 * reg * W2
    
    # compute b2 gradient
    grads['b2'] = torch.sum(dscores, dim=0)
    
    # Compute the w1 gradient
    dh1 = dscores.matmul(W2.T)
    dh1[h1 <= 0] = 0
    grads['W1'] = X.T.matmul(dh1) + 2 * reg * W1
    
    # Compute the b1 gradient
...
def nn_get_search_params():
    ...
    learning_rate_decays = [1e-3, 1e-2, 1e-1, 1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hidden_size in hidden_sizes:
            for regularization_strength in regularization_strengths:
                for learning_rate_decay in learning_rate_decays:
                    net = TwoLayerNet(X_train.shape[1], hidden_size, 10)
                    stat = net.train(X_train, y_train, X_val, y_val, learning_rate, learning_rate_decay, regularization_strength, num_iters=3000, batch_size=1000)
