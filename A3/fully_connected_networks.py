...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        w1 = torch.normal(mean = 0, std = weight_scale, size = (input_dim, hidden_dim), dtype = dtype, device = device)
        b1 = torch.zeros(hidden_dim, dtype = dtype, device = device)
        w2 = torch.normal(mean = 0, std = weight_scale, size = (hidden_dim, num_classes), dtype = dtype, device = device)
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        reshaped_x = X.reshape(X.shape[0], -1).to("cuda")
        a1 = reshaped_x.matmul(self.params['W1']) + self.params['b1']
        h1 = torch.relu(a1)
        a2 = h1.matmul(self.params['W2']) + self.params['b2']
        scores = a2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        da2, dw2, db2 = Linear.backward(dout, (h1, self.params['W2'], self.params['b2']))
        dh1 = ReLU.backward(da2, h1)
        da1, dw1, db1 = Linear.backward(dh1, (reshaped_x, self.params['W1'], self.params['b1']))
        grads = {'W1': dw1 + 2 * self.reg * self.params['W1'], 'b1': db1, 'W2': dw2 + 2 * self.reg * self.params['W2'], 'b2': db2}
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                self.params['W' + str(i + 1)] = torch.normal(mean = 0, std = weight_scale, size = (input_dim, hidden_dims[i]), dtype = dtype, device = device)
                self.params['b' + str(i + 1)] = torch.zeros(hidden_dims[i], dtype = dtype, device = device)
            elif i == self.num_layers - 1:
                self.params['W' + str(i + 1)] = torch.normal(mean = 0, std = weight_scale, size = (hidden_dims[i - 1], num_classes), dtype = dtype, device = device)
                self.params['b' + str(i + 1)] = torch.zeros(num_classes, dtype = dtype, device = device)
            else:
                self.params['W' + str(i + 1)] = torch.normal(mean = 0, std = weight_scale, size = (hidden_dims[i - 1], hidden_dims[i]), dtype = dtype, device = device)
                self.params['b' + str(i + 1)] = torch.zeros(hidden_dims[i], dtype = dtype, device = device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          if i == self.num_layers - 1:
            a, cache = Linear.forward(a, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
            caches.append(cache)
          else:
            a, cache = Linear_ReLU.forward(a, self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
            caches.append(cache)
            if self.use_dropout:
              a, cache = Dropout.forward(a, self.dropout_param)
              caches.append(cache)
        scores = a
        #################################################################
        #                      END OF YOUR CODE                         #
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, lr_decay=0.9, optim_config={'learning_rate':1e-1}, num_epochs=100, device='cuda')
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def adam(w, dw, config=None):
    ...
    next_w = w - config["learning_rate"] * m_hat / ((v_hat)**0.5 + config["epsilon"])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
