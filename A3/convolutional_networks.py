...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        #       out[:, f, i, j] = torch.sum(x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] * w[f], dim=(1, 2, 3)) + b[f]
        
        for i in range(H_out):
          for j in range(W_out):
            multi1 = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW].unsqueeze(1)
            multi2 = w.unsqueeze(0)
            out[:, :, i, j] = torch.sum(multi1 * multi2, dim=(2, 3, 4)) + b
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        #     for i in range(HH):
        #       for j in range(WW):
        #         dw[f, c, i, j] = torch.sum(dout[:, f, :, :] * x_pad[:, c, i:i+H_out*stride:stride, j:j+W_out*stride:stride])
        for i in range(HH):
            for j in range(WW):
              multi1 = dout.unsqueeze(2)
              multi2 = x_pad[:, :, i:i+H_out*stride:stride, j:j+W_out*stride:stride].unsqueeze(1)
              dw[:, :, i, j] = torch.sum(multi1 * multi2, dim=(0, 3, 4))
              
        
        # Calculate the gradient dx and dx_pad
        # for n in range(N):
        #   for f in range(F):
        #     for i in range(H_out):
        #       for j in range(W_out):
        ...
        for i in range(H_out):
          for j in range(W_out):
            multi1 = dout[:, :, i, j].unsqueeze(2).unsqueeze(3).unsqueeze(4)
            multi2 = w.unsqueeze(0)
            # print("multi1.shape: ", multi1.shape)
            # print("multi2.shape: ", multi2.shape)
            multi = multi1 * multi2
            # print("multi.shape: ", multi.shape)
            multi_sum = torch.sum(multi, dim=1)
            # print("multi_sum.shape: ", multi_sum.shape)
            # print("target shape", dx_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW].shape)
            dx_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += torch.sum(multi1 * multi2, dim=1)
            #dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += dout[n, f, i, j] * w[f] 
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for i in range(H_out):
          for j in range(W_out):
            sub_x = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            out[:, :, i, j] = torch.amax(sub_x, dim=(2, 3))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        #         x_max = torch.max(x_pool)
        #         dx[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] = (x_pool == x_max) * dout[n, c, i, j]
        for i in range(H_out):
          for j in range(W_out):
            x_pool = x[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            x_max = torch.amax(x_pool, dim=(2, 3)).unsqueeze(2).unsqueeze(3)
            dx[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] = (x_pool == x_max) * dout[:, :, i, j].unsqueeze(2).unsqueeze(3)
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        w1 = torch.normal(mean = 0, std = weight_scale, size = (num_filters, input_dims[0], filter_size, filter_size), dtype=dtype, device=device)
        b1 = torch.zeros(num_filters, dtype=dtype, device=device)
        w2 = torch.normal(mean = 0, std = weight_scale, size = (num_filters * input_dims[1] * input_dims[2] // 4, hidden_dim), dtype=dtype, device=device)
        b2 = torch.zeros(hidden_dim, dtype=dtype, device=device)
        w3 = torch.normal(mean = 0, std = weight_scale, size = (hidden_dim, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        out3, cache3 = MaxPool.forward(out2, pool_param)
        out4, cache4 = Linear.forward(out3, W2, b2)
        out5, cache5 = ReLU.forward(out4)
        out6, cache6 = Linear.forward(out5, W3, b3)
        scores = out6
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        ####################################################################
        # Replace "pass" statement with your code
        loss, dout6 = softmax_loss(scores, y)
        dout5, grads['W3'], grads['b3'] = Linear.backward(dout6, cache6)
        dout4 = ReLU.backward(dout5, cache5)
        dout3, grads['W2'], grads['b2'] = Linear.backward(dout4, cache4)
        dout2 = MaxPool.backward(dout3, cache3)
        dout1 = ReLU.backward(dout2, cache2)
        dx, grads['W1'], grads['b1'] = Conv.backward(dout1, cache1)
        ###################################################################
        #                             END OF YOUR CODE                    #
        ###################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers - 1):
          if i == 0:
            self.params["W" + str(i + 1)] = kaiming_initializer(input_dims[0], num_filters[i], K=3, relu = True, device = device, dtype = dtype)
            self.params["b" + str(i+1)] = torch.zeros(num_filters[i], dtype=dtype, device=device)
          else:
            self.params["W" + str(i+1)] = kaiming_initializer(num_filters[i-1], num_filters[i], K=3, relu = True, device = device, dtype = dtype)
            self.params["b" + str(i+1)] = torch.zeros(num_filters[i], dtype=dtype, device=device)
          if self.batchnorm:
        ...
        H_out = input_dims[1] // 2 ** len(max_pools)
        W_out = input_dims[2] // 2 ** len(max_pools)
       
        self.params["W" + str(self.num_layers)] = kaiming_initializer(num_filters[-1] * H_out * W_out, num_classes, K=None, relu = False, device = device, dtype = dtype)
    ...
    def loss(self, X, y=None):
        ...
        dout, grads["W" + str(self.num_layers)], grads["b" + str(self.num_layers)] = Linear.backward(dout, cache)
        grads["W" + str(self.num_layers)] += 2 * self.reg * self.params["W" + str(self.num_layers)]
        if not self.batchnorm:
          for i in range(self.num_layers - 2, -1, -1):
            if i in self.max_pools:
              dout, grads["W" + str(i+1)], grads["b" + str(i+1)] = Conv_ReLU_Pool.backward(dout, caches[i])
              grads["W" + str(i+1)] += 2 * self.reg * self.params["W" + str(i+1)]
            else:
              dout, grads["W" + str(i+1)], grads["b" + str(i+1)] = Conv_ReLU.backward(dout, caches[i])
              grads["W" + str(i+1)] += 2 * self.reg * self.params["W" + str(i+1)]
        else:
          for i in range(self.num_layers - 2, -1, -1):
            if i in self.max_pools:
              dout, grads["W" + str(i+1)], grads["b" + str(i+1)], grads["gamma" + str(i+1)], grads["beta" + str(i+1)] = Conv_BatchNorm_ReLU_Pool.backward(dout, caches[i])
              grads["W" + str(i+1)] += 2 * self.reg * self.params["W" + str(i+1)]
            else:
              dout, grads["W" + str(i+1)], grads["b" + str(i+1)], grads["gamma" + str(i+1)], grads["beta" + str(i+1)] = Conv_BatchNorm_ReLU.backward(dout, caches[i])
              grads["W" + str(i+1)] += 2 * self.reg * self.params["W" + str(i+1)]
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def find_overfit_parameters():
    ...
    weight_scale = 1e-1
    learning_rate = 1e-2
    print("weight_scale: ", weight_scale, "learning_rate: ", learning_rate)
    ###########################################################
    #                       END OF YOUR CODE                  #
    ###########################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    max_pools = [0, 1, 2, 3, 4]
    print("weight_scale: ", weight_scale, "learning_rate: ", learning_rate, "reg: ", reg, "num_filters: ", num_filters, "max_pools: ", max_pools)
    model = DeepConvNet(
      num_filters= num_filters,
      max_pools= max_pools,
      weight_scale=weight_scale, reg=reg, dtype=dtype, device=device)
    solver = Solver(model, data_dict, num_epochs=100, batch_size=128, 
                    update_rule=adam,
                    optim_config={
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.normal(mean = 0, std = (gain / Din) ** 0.5, size = (Din, Dout), dtype=dtype).to(device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        # Replace "pass" statement with your code
        fan_in = Din * K * K
        weight = torch.normal(mean = 0, std = (gain / fan_in) ** 0.5, size = (Dout, Din, K, K), dtype = dtype).to(device)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        #step4
        dsq = 1. /N * torch.ones(N , D, device = dout.device, dtype = dout.dtype) * dvar
        ...
        #step1
        dx2 = 1. /N * torch.ones(N , D, device = dout.device, dtype = dout.dtype) * dmu
